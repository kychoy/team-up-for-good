...
serve(async (req) => {
  ...
  try {
    ...
    const { elderly_profile_id, contact_id, method, message }: SendAlertRequest = await req.json();
    ...
    try {
      if (method === 'email' && contact.email) {
        ...
        alertStatus = 'sent';
      } else if (method === 'voice' && contact.phone) {
        ...
        throw new Error(`Invalid method or missing contact information for ${method}`);
      }
    ...
    return new Response(
      JSON.stringify({ 
        ...
        status: alertStatus,
        message: alertStatus === 'sent' ? 'Alert sent successfully' : 'Alert failed to send',
        error: errorMessage 
      }),
      { status: alertStatus === 'sent' ? 200 : 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error: any) {
